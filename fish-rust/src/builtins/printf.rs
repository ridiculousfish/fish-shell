...
use crate::wutil::wcstoi::fish_wcstoi_partial;
...
trait RawStringToScalarType: Copy + num_traits::Zero + std::convert::From<u32> {
    ...
    fn raw_string_to_scalar_type<'a>(s: &'a wstr, end: &mut &'a wstr) -> Result<Self, Error>;
...
impl RawStringToScalarType for i64 {
    fn raw_string_to_scalar_type<'a>(s: &'a wstr, end: &mut &'a wstr) -> Result<Self, Error> {
        let mut consumed = 0;
        let res = fish_wcstoi_partial(s, &mut consumed);
...
impl RawStringToScalarType for f64 {
    fn raw_string_to_scalar_type<'a>(s: &'a wstr, end: &mut &'a wstr) -> Result<Self, Error> {
        ...
        let mut result = wcstod(s, get_decimal_sep(), &mut consumed);
...
fn string_to_scalar_type<'a, T: RawStringToScalarType>(
    s: &wstr,
    state: &mut builtin_printf_state_t<'a>,
) -> T {
    if s.char_at(0) == '"' || s.char_at(0) == '\'' {
        ...
        let mut end = s;
        let mval = T::raw_string_to_scalar_type(s, &mut end);
...
impl<'a> builtin_printf_state_t<'a> {
    fn verify_numeric(&mut self, s: &wstr, end: &wstr, errcode: Option<Error>) {
        if errcode != None && errcode != Some(Error::InvalidChar) {
    ...
    fn print_direc(
        ...
        match conversion {
            'd' | 'i' => {
                ...
                if !have_field_width {
                    if !have_precision {
                        self.append_output_str(sprintf!(fmt, arg));
                    } else {
                        self.append_output_str(sprintf!(fmt, precision, arg));
                    }
                } else {
                    if !have_precision {
                        self.append_output_str(sprintf!(fmt, field_width, arg));
                    } else {
                        self.append_output_str(sprintf!(fmt, field_width, precision, arg));
                    }
                }
            }
            'o' | 'u' | 'x' | 'X' => {
            ...
            'c' => {
                if !have_field_width {
                    self.append_output_str(sprintf!(fmt, argument.char_at(0)));
                } else {
                    self.append_output_str(sprintf!(fmt, field_width, argument.char_at(0)));
                }
            }
            ...
            's' => {
                if !have_field_width {
                    if !have_precision {
                        self.append_output_str(sprintf!(fmt, argument));
                    } else {
                        self.append_output_str(sprintf!(fmt, precision, argument));
                    }
                } else {
                    if !have_precision {
                        self.append_output_str(sprintf!(fmt, field_width, argument));
                    } else {
                        self.append_output_str(sprintf!(fmt, field_width, precision, argument));
                    }
                }
            }
    ...
    /// Return the remainder of the string after the escape sequence.
    /// If octal_0 is nonzero, octal escapes are of the form \0ooo, where o
    /// is an octal digit; otherwise they are of the form \ooo.
    fn print_esc<'b>(&mut self, escstart: &'b wstr, octal_0: bool) -> &'b wstr {
    ...
    fn print_esc_string(&mut self, mut str: &wstr) {
        while !str.is_empty() {
            let c = str.char_at(0);
            if c == '\\' {
                let rest = self.print_esc(str, true);
                // Ensure we advanced the string.
                assert!(wstr_offset_in(rest, str) > 0);
                str = rest;
            } else {
                self.append_output(c);
                str = &str[1..];
...
pub fn printf(
    ...
    // RUST-TODO: need to implement fish-numeric-locale.
