...
static bool fork_child_for_process(parser_t &parser, const std::shared_ptr<job_t> &job,
                                   process_t *p, const dup2_list_t &dup2s, const char *fork_type,
                                   const std::function<void()> &child_action) {
    bool claim_terminal = job->should_claim_terminal() && parser.get_job_tree().is_focused();
    ...
    parser.get_job_tree().set_foreground_job(job.get(), false);
    return true;
}
...
static bool exec_external_command(parser_t &parser, const std::shared_ptr<job_t> &j, process_t *p,
    ...
    if (use_posix_spawn) {
        ...
        parser.get_job_tree().set_foreground_job(j.get(), false);
    } else
#endif
    {
        if (!fork_child_for_process(parser, j, p, dup2s, "external command",
...
static bool exec_concurrent_block_or_func_process(parser_t &parser, std::shared_ptr<job_t> j,
    ...
    if (auto performer = get_performer_for_process(p, j.get(), io_chain)) {
        ...
        std::shared_ptr<parser_t> bg_executor = parser.branch(j->job_tree);
