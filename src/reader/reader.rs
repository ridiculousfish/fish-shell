...
use crate::{
    ...
    history::{
        History, HistoryId, HistoryItem, HistoryItemId, HistorySearch, MemoryHistoryId,
        PersistenceMode, SearchDirection, SearchFlags, SearchType, history_id, in_private_mode,
    },
    input::{
...
fn read_i(parser: &mut Parser) {
    ...
    while !check_exit_loop_maybe_warning(Some(&mut reader)) {
        ...
        let Some((command, history_item_id)) =
            reader.readline(set_shell_modes_temporarily(reader.conf.inputfd), None)
        else {
            continue;
        };
        ...
        if let Some(id) = history_item_id {
            ...
            reader.history.emit_update(update);
        }
