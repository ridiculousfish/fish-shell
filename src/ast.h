...
#ifndef FISH_PARSE_GRAMMAR_2_H
#define FISH_PARSE_GRAMMAR_2_H
 ...
 *
 * The ast is allocated using an "arena allocator" so that it may be
 * deallocated efficiently. This means that crucially nodes must not
 * have destructors and must be trivially copyable. Furthermore they
 * do have no virtual functions.
 *
 ...
 *
 *    /// A field may be a "pointer embedding" of a node.
 *    /// Every such pointer must be non-null after construction.
 *    template <typename Node>
 *    void visit_pointer_field(Node *&ptr);
 *
...
struct union_ptr_t {
    ...
    /* implicit */ inline union_ptr_t(node_t *contents = nullptr) : contents(contents) {
...
struct node_t {
    ...
    /// We are allocated via an arena allocator.
...
// These are allocated with the arena allocator so no memory management is required here.
...
struct list_t : public node_t {
    ...
    // We need double-indirection here because we don't know the count until after we have populated
    // as much as we can. contents points at an array of non-null pointers.
    ...
    const ContentsNode *at(size_t idx, bool reverse = false) const {
        ...
        return contents[reverse ? count - idx - 1 : idx];
    }
    ...
    /// Iteration support.
    const ContentsNode *const *begin() const { return contents; }
    const ContentsNode *const *end() const { return contents + count; }
...
#define ELEMLIST(ListT, ContentsT) \
    struct ListT##_t : public list_t<type_t::ListT, ContentsT##_t> {};
#include "ast_node_types.inc"
...
struct keyword_t : public keyword_base_t {
...
struct maybe_newlines_t : public leaf_t<type_t::maybe_newlines> {};
...
struct variable_assignment_t : public leaf_t<type_t::variable_assignment> {};

// An argument or redirection holds either an argument or redirection.
struct argument_or_redirection_t : public branch_t<type_t::argument_or_redirection> {
...
struct statement_t : public branch_t<type_t::statement> {
...
struct job_t : public branch_t<type_t::job> {
...
struct job_conjunction_t : public branch_t<type_t::job_conjunction> {
...
struct for_header_t : public branch_t<type_t::for_header> {
    // 'for'
...
struct while_header_t : public branch_t<type_t::while_header> {
    // 'while'
...
struct function_header_t : public branch_t<type_t::function_header> {
...
struct begin_header_t : public branch_t<type_t::begin_header> {
...
struct block_statement_t : public branch_t<type_t::block_statement> {
...
struct if_clause_t : public branch_t<type_t::if_clause> {
    // The 'if' keyword.
...
struct elseif_clause_t : public branch_t<type_t::elseif_clause> {
    // The 'else' keyword.
...
struct else_clause_t : public branch_t<type_t::else_clause> {
    // else ; body
    keyword_t<parse_keyword_t::kw_else> kw_else;
    semi_nl_t semi_nl;
    job_list_t body;
...
struct if_statement_t : public branch_t<type_t::if_statement> {
    // if part
    if_clause_t if_clause;
...
struct case_item_t : public branch_t<type_t::case_item> {
    // case <arguments> ; body
...
struct switch_statement_t : public branch_t<type_t::switch_statement> {
...
struct decorated_statement_t : public branch_t<type_t::decorated_statement> {
...
struct not_statement_t : public branch_t<type_t::not_statement> {
...
struct job_continuation_t : public branch_t<type_t::job_continuation> {
...
struct job_conjunction_continuation_t : public branch_t<type_t::job_conjunction_continuation> {
    // The && or || token.
...
// instances of this.
struct andor_job_t : public branch_t<type_t::andor_job> {
...
struct freestanding_argument_list_t : public branch_t<type_t::freestanding_argument_list> {
    argument_list_t arguments;
    FIELDS(arguments)
};
...
class ast_t {
   public:
    ...
    traversal_t walk() const { return traversal_t{top_}; }

    /// \return the top node. This has the type requested in the 'parse' method.
    const node_t *top() const { return top_; }
    ...
    struct extras_t {
        ...
        /// Set of newlines, sorted by offset.
        source_range_list_t newlines;
    ...
    iterator begin() const { return iterator{top_}; }
   ...
   private:
    explicit ast_t(std::unique_ptr<arena_alloc_t> &&mem) : mem_(std::move(mem)) {}
    ...
    // Its memory is owned by our arena allocator.
    const node_t *top_{};
    ...
    // The owner of memory for all of the nodes.
    std::unique_ptr<arena_alloc_t> mem_;
