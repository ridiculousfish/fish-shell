...
static int path_expand(parser_t &parser, io_streams_t &streams, int argc, const wchar_t **argv) {
    options_t opts;
    ...
    while (const wcstring *arg = aiter.nextstr()) {
        // TODO: Error handling.
        // Unfortunately `glob()` does not support `**` recursion.
        // So we'd have to add it ourselves (tricky) or write our own globbing (annoying and tricky).
        // Instead let's just fail out now, if we ever add it that can go away.
        size_t pos;
        if ((pos = arg->find(L"**")) != wcstring::npos) {
            if (count_preceding_backslashes(*arg, pos) % 2 == 0) {
                streams.err.append(L"Path does not support '**' wildcards at the moment");
                continue;
            }
        }

        std::string narrow = wcs2string(*arg);
        glob_t globs;
        int glob_flags = 0;
        // GLOB_MARK - add "/" for matched directories
        // We need to remove this if the pattern already does or we get "//".
        if (!string_suffixes_string(L"/", *arg)) glob_flags |= GLOB_MARK;
        // Maybe also:
        // GLOB_NOSORT - then do our own sorting
        glob(narrow.c_str(), glob_flags, nullptr, &globs);
        if (opts.quiet && globs.gl_pathc > 0) {
            globfree(&globs);
            return STATUS_CMD_OK;
        }
        for (uint i = 0; i < globs.gl_pathc; i++) {
            wcstring path = str2wcstring(globs.gl_pathv[i]);
            if (filter_path(opts, path)) {
                path_out(streams, opts, path);
                n_transformed++;
            }
        }
        globfree(&globs);
...
// `path filter` - like expand, but *just* the filtering, no globbing.
