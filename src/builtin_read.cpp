...
static int read_interactive(wcstring &buff, const environment_t &vars, int nchars, bool shell,
...
int builtin_read(parser_t &parser, io_streams_t &streams, wchar_t **argv) {
    ...
    do {
        ...
        if (stream_stdin_is_a_tty && !opts.split_null) {
            ...
            exit_res = read_interactive(buff, parser.vars(), opts.nchars, opts.shell, opts.silent,
