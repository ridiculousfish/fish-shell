...
static bool autosuggest_parse_command(const wcstring &buff, const operation_context_t &ctx,
    ...
    if (const ast::job_conjunction_t *jc = ast.top()->as<ast::job_list_t>()->at(0)) {
        first_statement = jc->job.statement.try_as<ast::decorated_statement_t>();
    }
...
class highlighter_t {
   ...
   public:
    ...
    highlighter_t(const wcstring &str, size_t pos, const operation_context_t &ctx, wcstring wd,
                  bool can_do_io)
        : buff(str), cursor_pos(pos), ctx(ctx), io_ok(can_do_io), working_directory(std::move(wd)) {
        auto astflags = parse_flag_continue_after_error | parse_flag_include_comments |
                        parse_flag_accept_incomplete_tokens | parse_flag_leave_unterminated |
                        parse_flag_show_blank_lines | parse_flag_show_extra_semis;
        ast = make_unique<ast::ast_t>(ast::ast_t::parse(buff, astflags));
    }
