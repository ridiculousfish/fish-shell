...
static bool write_color_escape(outputter_t &outp, char *todo, unsigned char idx, bool is_fg) {
    if (term_supports_color_natively(idx)) {
        // Use tparm to emit color escape.
        writembs(outp, tparm(todo, idx));
        return true;
    }
...
void outputter_t::flush_to(int fd) {
    if (fd >= 0 && !buffer_.empty()) {
        write_loop(fd, buffer_.data(), buffer_.size());
        buffer_.clear();
    }
}
...
// recursive.
static fish_mutex_t s_tputs_receiver_lock;
...
void writembs_check(outputter_t &outp, char *mbs, const char *mbs_name, bool critical, const char *file, long line) {
    if (mbs != NULL) {
