...
class parser_t : public std::enable_shared_from_this<parser_t> {
   ...
   private:
    ...
    const std::shared_ptr<pgid_selector_t> pgid_selector;
    ...
    /// Create a parser.
    parser_t(std::shared_ptr<env_stack_t> vars, std::shared_ptr<pgid_selector_t> pgid_selector);
   ...
   public:
    ...
    std::shared_ptr<parser_t> branch(const std::shared_ptr<pgid_selector_t> &pg) const;
