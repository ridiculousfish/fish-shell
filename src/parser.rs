...
/// An RAII-type object which holds a saved [`ScopedData`] and restores the scoped data
/// in its Drop implementation.
#[must_use = "Must save ParserScope in variable to extend its scope."]
pub struct ParserScope {
    data: Rc<Cell<ScopedData>>,
    saved: ScopedData,
}

impl Drop for ParserScope {
    fn drop(&mut self) {
        self.data.set(self.saved);
    }
}
...
pub struct Parser {
    ...
    scoped_data: Rc<Cell<ScopedData>>,
...
impl Parser {
    ...
    pub fn new(variables: Rc<EnvStack>, cancel_behavior: CancelBehavior) -> Parser {
        let result = Self {
            ...
            variables,
            scoped_data: Rc::new(Cell::new(ScopedData::default())),
    ...
    /// data when dropped.
    pub fn push_scope<F: FnOnce(&mut ScopedData)>(&self, modifier: F) -> ParserScope {
        let data = Rc::clone(&self.scoped_data);
        let saved = data.get();
        let mut modified = saved;
        modifier(&mut modified);
        data.set(modified);
        ParserScope { data, saved }
    }
