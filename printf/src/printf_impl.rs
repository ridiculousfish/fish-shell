...
pub enum Error {
    BadFormatString, // invalid format string
    MissingArg,      // too few args
    ExtraArg,        // too many args
    BadArgType,      // arg type doesn't match format specifier
    Overflow,        // e.g. precision is too large to represent
    Fmt(fmt::Error), // error emitted by the output stream
}

// Convenience conversion from from fmt::Error.
...
pub fn sprintf_locale(
    ...
    'main: while !s.is_empty() {
        ...
        if prec.is_some() && prec.unwrap() > i32::MAX as usize {
