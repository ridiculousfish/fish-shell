...
use crate::fishffi::{ffi, str2wcstring_ffi, wcstring_ptr};
use crate::wchar::{wcharz_ptr_t, L};
use nix::errno::Errno;
use nix::unistd;
...
mod topic_monitor_ffi {
    ...
    extern "C++" {
        include!("common.h");
        type wcstring = crate::fishffi::ffi::wcstring;
    }
...
use topic_monitor_ffi::{generation_list_t, topic_t};
...
pub struct binary_semaphore_t {
    ...
    sem_: Box<UnsafeCell<libc::sem_t>>,
...
impl binary_semaphore_t {
    pub fn new() -> binary_semaphore_t {
        ...
        #[allow(unused_mut)]
        let mut sem_ = Box::new(UnsafeCell::new(libc::sem_t::default()));
        ...
        #[cfg(target_os = "linux")]
        {
            let res = unsafe { libc::sem_init(sem, 0, 0) };
            sem_ok_ = res == 0;
        }
        if !sem_ok_ {
            ...
            if cfg!(feature = "FISH_TSAN_WORKAROUNDS") {
                ffi::make_fd_nonblocking(pipes_.read.fd());
            }
        }
        binary_semaphore_t {
            sem_ok_,
    ...
    pub fn die(&self, msg: wcharz_ptr_t) {
        ffi::wperror(msg);
        panic!("die");
    }
}
...
impl Drop for binary_semaphore_t {
    fn drop(&mut self) {
        ...
        {
            if self.sem_ok_ {
                _ = unsafe { sem_destroy(self.sem_.get()) };
            }
        }
    }
}
...
impl topic_monitor_t {
    ...
    fn check(&self, gens: *mut generation_list_t, wait: bool) -> bool {
